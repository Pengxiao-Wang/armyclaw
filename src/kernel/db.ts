...
export function createTask(
  task: Omit<Task, 'created_at' | 'updated_at' | 'context_chain' | 'error_count' | 'source_message_id' | 'complexity' | 'timeout_sec' | 'delivery_content'> & {
    created_at?: string;
    updated_at?: string;
  ...
  const row: Task = {
    ...
    delivery_content: task.delivery_content ?? null,
  ...
  const insertTaskTxn = db.transaction(() => {
    db.prepare(`
      INSERT INTO tasks (id, parent_id, campaign_id, state, description, priority, assigned_agent, assigned_engineer_id, intent_type, reject_count_tactical, reject_count_strategic, rubric, artifacts_path, error_count, override_skip_gate, source_channel, source_chat_id, source_message_id, complexity, timeout_sec, delivery_content, context_chain, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      ...
      row.source_channel, row.source_chat_id, row.source_message_id, row.complexity, row.timeout_sec, row.delivery_content, row.context_chain,
...
const TASK_UPDATE_FIELDS = new Set([
  ...
  'source_channel', 'source_chat_id', 'source_message_id', 'timeout_sec', 'delivery_content', 'context_chain',
]);
