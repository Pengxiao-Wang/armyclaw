...
// webhook credentials. Zero external deps.
// ═══════════════════════════════════════════════════════════
...
async function main(): Promise<void> {
  ...
  const verificationToken = await ask('Verification Token (from Event Subscription)');
  const encryptKey = await ask('Encrypt Key (optional, leave empty to skip)', '');

  // Step 3: DM Policy
  print('');
  print('── Step 3: Message Policy ─────────────────────────────');
  print('');
  ...
  // Step 4: Webhook
  print('');
  print('── Step 4: Webhook Configuration ─────────────────────');
  print('');

  const webhookPort = await ask('Webhook port', '3003');
  const webhookPath = await ask('Webhook path', '/webhook/event');
  ...
  print('');
  print('2. Set the webhook URL in Lark Open Platform:');
